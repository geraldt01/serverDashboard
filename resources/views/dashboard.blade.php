...
@section('content')
    ...
    <section class="meters">
        ...
        <article class="panel meter"><h3>Servers missing patches</h3><div class="value">{{ number_format($ec2MissingPatches) }}</div></article>
        <article class="panel meter"><h3>Webpage issues found</h3><div class="value">{{ number_format($webpageIssues) }}</div></article>
    </section>
    ...
    <section class="grid">
        ...
        <article class="panel content"><h2>WordPress Plugin Updates</h2><div class="scroll"><table id="pluginsTable" data-paginate="10"><thead><tr><th>Site</th><th>Plugin</th><th>Current</th><th>Latest</th><th>Status</th></tr></thead><tbody>@forelse($plugins as $plugin)<tr><td>{{ $plugin->site_name }}</td><td>{{ $plugin->plugin_name }}</td><td>{{ $plugin->current_version }}</td><td>{{ $plugin->latest_version }}</td><td><span class="badge {{ $plugin->status === 'outdated' ? 'warning' : 'ok' }}">{{ $plugin->status }}</span></td></tr>@empty<tr><td colspan="5">No plugin reports received.</td></tr>@endforelse</tbody></table></div><div class="pagination" data-pagination-for="pluginsTable"></div></article>
        <article class="panel content"><h2>Server Patch Status</h2><div class="scroll"><table id="ec2Table" data-paginate="5"><thead><tr><th>Source</th><th>Instance</th><th>OS Version</th><th>Missing</th><th>Security</th><th>Installed</th><th>Failed</th><th>Reboot</th><th>Last Checked</th></tr></thead><tbody>@forelse($instances as $instance)<tr><td><span class="badge {{ $instance->source === 'Agent' ? 'ok' : '' }}">{{ $instance->source }}</span></td><td>{{ $instance->name }}<br><span class="muted">{{ $instance->identifier }}</span></td><td>{{ $instance->os_version ?? '—' }}</td><td>{{ $instance->missing_count }}</td><td><span class="badge {{ $instance->security_count > 0 ? 'danger' : 'ok' }}">{{ $instance->security_count }}</span></td><td>{{ $instance->installed_count ?? '—' }}</td><td>{{ $instance->failed_count ?? '—' }}</td><td><span class="badge {{ $instance->reboot_required ? 'warning' : 'ok' }}">{{ $instance->reboot_required ? 'required' : 'no' }}</span></td><td>{{ optional($instance->checked_at)->diffForHumans() ?? 'never' }}</td></tr>@empty<tr><td colspan="9">No patch reports received.@if(auth()->user()->isAdmin()) <a href="{{ route('other-servers.index') }}">Register a server</a> to start tracking updates.@endif</td></tr>@endforelse</tbody></table></div><div class="pagination" data-pagination-for="ec2Table"></div></article>
        ...
        <article class="panel content"><h2>Frontend Page Checks</h2><div class="scroll"><table id="webpageChecksTable" data-paginate="5"><thead><tr><th>Page</th><th>Status</th><th>HTTP</th><th>Response</th><th>Broken images</th><th>Broken videos</th><th>Missing elements</th><th>Last checked</th></tr></thead><tbody>@forelse($webpageChecks as $check)<tr><td><strong>{{ $check->name }}</strong><br><span class="muted">{{ $check->url }}</span></td><td><span class="badge {{ $check->last_status === 'healthy' ? 'ok' : ($check->last_status === 'warning' ? 'warning' : ($check->last_status === 'broken' ? 'danger' : '')) }}">{{ $check->last_status }}</span></td><td>{{ $check->last_http_status ?? '—' }}</td><td>{{ $check->last_response_time_ms !== null ? $check->last_response_time_ms . ' ms' : '—' }}</td><td>{{ $check->broken_images_count }}</td><td>{{ $check->broken_videos_count }}</td><td>{{ $check->missing_elements_count }}</td><td>{{ optional($check->last_checked_at)->diffForHumans() ?? 'never' }}</td></tr>@empty<tr><td colspan="8">No webpages registered for frontend checks.@if(auth()->user()->isAdmin()) <a href="{{ route('webpage-checks.index') }}">Add a webpage</a> to start checking.@endif</td></tr>@endforelse</tbody></table></div><div class="pagination" data-pagination-for="webpageChecksTable"></div></article>
    </section>
    <script>
